...
async function pendingQueue(fromDate){const c=supa();if(!c?.from)return[];try{const r=await c.from('limpieza_plan').select('ac_id,prioridad,estado,fecha').gte('fecha',fromDate).eq('estado','pendiente').order('fecha',{ascending:true}).order('prioridad',{ascending:true}).order('ac_id',{ascending:true}).limit(500);if(r.error)return[];return (r.data||[]).map(x=>({ac:String(x.ac_id).toUpperCase(),date:String(x.fecha).slice(0,10),prioridad:x.prioridad==null?999999:Number(x.prioridad),estado:x.estado}))}catch{return[]}}
